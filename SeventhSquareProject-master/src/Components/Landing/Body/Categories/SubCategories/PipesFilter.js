...
function PipesFilter(props) {
  const [pipesize,setpipesize]=useState([]);
  let pipes=[]
  const fetchpipesize=async()=>{
    ...
    const response = await fetch(
      "https://seller.seventhsq.com/inventory/api/inventory_detail_by_category/Pipes_&_Fittings/"+city
    );
    const data= await response.json();
    data.forEach(myFunction)
    function myFunction(item) {
      pipes.push(item.length)
      let unique=[...new Set(pipes)]
      setpipesize(unique)
                           ...
                           <div>
                               <div class="form-check pl-0 mb-3 mx-4">
                                <input
                                  ...
                                  id={curr}
                                  onChange={()=>props.handlepipesize(curr)}
                                  // defaultChecked={props.pipesizes.includes(curr)?true:false}
                                />
                                <label
