...
function Checkout_po() {
  ...
  const getaddressdata = () => {
    ...
    axios.get(`https://api.seventhsq.com/enquiry/get_rfq_address/${id}`, config).then(
      (res) => {
        ...
        setstreet(res.data[0].delivery_location);

        setcity(res.data[0].city);
        setstate(res.data[0].state);
        setzip(res.data[0].pincode);
  ...
  const getdata = async () => {
    ...
    const res = await fetch(
      `https://api.seventhsq.com/enquiry/get_approved_po/${id}`,
      config
    );

    const data = await res.json();
    console.log("po app data");
    console.log(data);
    setdata(data);
    let s = 0;
...
export default withRouter(Checkout_po);
